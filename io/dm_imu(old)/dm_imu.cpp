...
void DM_IMU::get_imu_data_thread()
{
  while (!stop_thread_) {
    ...
    if (receive_data.FrameHeader1 == 0x55 && receive_data.flag1 == 0xAA) {
      ...
      if (receive_data.slave_id1 == 0x01 && receive_data.reg_acc == 0x01) {
      ...
      else if (receive_data.slave_id1 == 0x01 && receive_data.reg_acc == 0x03) {
        serial_.read((uint8_t *)(&control_data.bullet_speed_u16), sizeof(Control_Receive_Frame) - 4);
        
        if (tools::get_crc16((uint8_t *)(&control_data.FrameHeader), sizeof(Control_Receive_Frame) - 3) == control_data.crc) {
          bullet_speed = control_data.bullet_speed_u16 / 1e2;
          mode = Mode(control_data.mode);
          shoot_mode = ShootMode(control_data.shoot_mode);
          ft_angle = control_data.ft_angle_u16 / 1e4;
          ...
          if (bullet_speed > 0 && tools::delta_time(now, last_log_time) >= 1.0) {
            tools::logger()->info(
              "[DM_IMU] Bullet speed: {:.2f} m/s, Mode: {}, Shoot mode: {}, FT angle: {:.2f} rad",
      ...
      tools::logger()->debug("[DM_IMU] invalid frame header");
    }
  }
}
